...
public:
    ...
    int count_larger(K x) {
        splitByKey(root, make_pair(x.first, x.second + 1), l, r);
        int res = size(r);
        join(root, l, r);
        return res;
    }
...
int dfs_find_centroid(int u, int p, int total) {
    //cerr << "dfs_find_centroid " << u << ' ' << p << ' ' << total << endl;
    int max_size = 0;
    int to = 0;
    for (auto e : a[u]) {
        int v = e.first;
...
void dfs_for_centroid(int root, int u, int p, int sum_w) {
    ...
    //cerr << "dfs_for_centroid " << root << ' ' << u << ' ' << res[root] << endl;
    for (auto e : a[u]) {
        int v = e.first;
        int w = e.second;
        if (erased[v]) continue;
...
namespace naive {
    ...
    void solve() {
        for (int i = 1; i <= n; ++i) if (!has_shop[i]) {
            has_shop[i] = true;
            dijkstra(d);
            res[i] = 0;
            for (int j = 1; j <= n; ++j) {
...
void gen() {
    ...
    for (int i = 2; i <= n; ++i) {
        int j = rand() % (i - 1) + 1;
        int w = rand() % LIM_WEIGHT + 1;
        a[i].push_back(make_pair(j, w));
        a[j].push_back(make_pair(i, w));
    }
    for (int i = 1; i <= n; ++i) {
        if (rand() % 100 < PROB) {
            has_shop[i] = true;
...
/*
        naive::solve();
        for (int i = 1; i <= n; ++i) if (!has_shop[i]) {
//            cerr << i << ' ' << res[i] << ' ' << naive::res[i] << endl;
            if (res[i] != naive::res[i]) {
                cerr << "BEGIN TESTCASE\n";
                cerr << n << endl;
                for (int i = 1; i <= n; ++i) {
                    for (auto e : a[i]) {
                        if (e.first > i) {
                            cerr << i << ' ' << e.first << ' ' << e.second << endl;
                        }
                    }
                }
                for (int i = 1; i <= n; ++i) {
                ...
                cerr << endl << "END TESTCASE\n";
            }
            assert(res[i] == naive::res[i]);
        }
*/
        reset();
    }
